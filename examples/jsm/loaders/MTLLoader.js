...
class MaterialCreator {
	...
	createMaterial_( materialName ) {
		...
		if ( params.metalness !== undefined /* || params.transparent*/ ) {
			...
			const blob = b64toBlob( realData, dataType );
			for ( let i = 0; i < 6; i ++ ) {
			...
			// env map is set in engine, here only the envMapIntensity is set to tell the app logic to use the env map
			this.materials[ materialName ].envMap = reflectionCube;
			//this.materials[ materialName ].combine = MultiplyOperation;
			this.materials[ materialName ].envMapIntensity = 4.0;
			this.materials[ materialName ].reflectivity = 4.8;
